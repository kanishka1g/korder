<template>
	<Page title="Habits Dashboard" :error="error" variant="dashboard" class="habits-dashboard">
		<template #actions>
			<VBtn
				color="primary"
				variant="elevated"
			...
			<VCard v-if="viewModal.habit.checkIns.length" class="progress-card mb-4" variant="tonal" color="info">
				<VCardText class="pa-4">
					<div class="d-flex align-center justify-space-between">
						...
						<VChip
							:color="
								viewModal.habit.checkIns.filter((c) => !c.checked).length === 0 ? 'success' : 'warning'
							"
							variant="elevated"
						>
...
import { parseDate } from "@/utils/time";
    ...
    function handleEditNote(habit) {
    	habit.missedNote = habit.checkIns.find((c) => dayjs(c.date).isSame(filterDate.value, "day"))?.missedNote || null;
    ...
    function handleViewStats(stat) {
    	...
    	statsModal.value.type = stat.type;
...
@media (max-width: 600px) {
	...
	.habit-details-card {
		...
		&:hover {
			transform: translateY(-1px);
			box-shadow: 0 4px 16px rgba(var(--v-theme-info), 0.1);
		}
	}
		...
		.checkins-list {
			.checkin-item {
				...
				&.checkin-missed {
					...
					background: linear-gradient(90deg, rgba(var(--v-theme-error), 0.05) 0%, transparent 100%);
				}
			}
		}
	}

	// Theme-specific modal content styles
	.v-theme--dark {
		.weekdays-card {
